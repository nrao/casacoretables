...
# MeasurementSet helpers live in casacore's `ms` module, which this
# tables-only package does not bundle. Import them if available; otherwise the
# MS-specific tests below are skipped.
try:
    from casacoretables.tables import (
        msconcat,
        removeDerivedMSCal,
...
class TestTable(unittest.TestCase):
    ...
    def test_tableinfo(self):
        ...
        t = table(
            "ttable.py_tmp.tab1", maketabdesc((c1, c2, c3, c4, c5, c6)), ack=False
        )
        ...
        self.assertTrue(
            compare(t.colnames(), ["cols", "colc", "coli", "cold", "colb", "colarr"])
        )
        self.assertEqual(
            tableinfo("ttable.py_tmp.tab1"), {"readme": "", "subType": "", "type": ""}
        )
    ...
    def test_tableascii(self):
        ...
        t = table("ttable.py_tmp.tab1", maketabdesc((c1, c2, c3, c4, c5)), ack=False)
        tcol = t.colnames()
        t.addrows(5)
    ...
    def test_check_putdata(self):
        ...
        np.testing.assert_array_equal(t.getcol("cold"), np.array([0.0, 0.0]))
    ...
    def test_adddmcolumns(self):
        ...
        t.addcols(
            maketabdesc(makescacoldesc("coli2", 0)),
        ...
        t.addcols(
            maketabdesc(makearrcoldesc("colarrtsm", 0.0 + 0j, ndim=2)),
        ...
        coldmi["NAME"] = "tsm2"
        t.addcols(maketabdesc(makearrcoldesc("colarrtsm2", 0.0, ndim=2)), coldmi)
    ...
    def test_taqlcalc(self):
        ...
        t.addrows(2)
        np.testing.assert_array_almost_equal(t.calc("(1 km)cm"), np.array([100000.0]))
    ...
    @unittest.skipUnless(HAVE_MS, "ms module not bundled")
    def test_msutil(self):
        """Testing msutil."""
        datacoldesc = makearrcoldesc("DATA", 0.0, ndim=2, shape=[20, 4])
        ...
        taql(
            "SELECT FROM tabtemp where TIME in (SELECT DISTINCT TIME"
    ...
    def test_hypercolumn(self):
        ...
        acd2 = makearrcoldesc("arr2", 0.0 + 0j)
    ...
    def test_complete_desc(self):
        ...
        for i, name in enumerate(("MAIN",) + subtables):
            ...
            with table(
                "complete_desc_table_%s-%d.table" % (name, i),
                desc,
                ack=False,
                readonly=False,
            ) as T:
    ...
    def test_required_desc(self):
        ...
        with default_ms("ttable.py_tmp.ms2", ms2_desc, ms2_dminfo) as ms2:
            ...
            self.assertTrue(
                np.all(dminfo["SPEC"]["HYPERCUBES"]["*1"]["TileShape"] == [3, 128 * 64])
            )
            ...
            self.assertTrue(
                np.all(dminfo["SPEC"]["HYPERCUBES"]["*1"]["TileShape"] == [4, 16, 32])
            )
        ...
        for c in subtables:
            ...
            with default_ms_subtable(c, subtname, tabdesc, dminfo) as subt:
                ...
                self.assertTrue(
                    np.all(
                        dminfo["SPEC"]["HYPERCUBES"]["*1"]["TileShape"] == [4, 16, 32]
                    )
                )
